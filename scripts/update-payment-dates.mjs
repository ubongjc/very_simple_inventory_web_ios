 ...
 * Run with: node scripts/update-payment-dates.mjs
 */
  ...
  try {
    ...
    for (const booking of bookings) {
      ...
      if (shouldUpdate) {
        ...
        const oldDate = booking.paymentDueDate
          ? booking.paymentDueDate.toISOString().split('T')[0]
          : 'null';
        const newDate = booking.endDate.toISOString().split('T')[0];

        console.log(
          `✓ Updated ${booking.reference || booking.id}: ${oldDate} → ${newDate}`
        );
        updatedCount++;
      } else {
        skippedCount++;
